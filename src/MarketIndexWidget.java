...
public class MarketIndexWidget implements StockWidget {
     ...
     */
    MarketIndexWidget(double size, boolean buttonActive) {
         this.size = size;
        ...
        displayQuarter = new GraphicsText();
        displayQuarter.setFont(FontStyle.BOLD, size * 0.02);
        group.add(displayQuarter);

        displayPortfolio= new GraphicsText();
        displayPortfolio.setFont(FontStyle.BOLD, size * 0.02);
        group.add(displayPortfolio);
        ...
        advanceTime.onClick(() -> {
            if (quarter <= 20) {
                quarter++;
                if(quarter == 19)   {
                    buttonGroup.remove(advanceTime);
                }
            }
            else {
                return;
    ...
    public void update() {
        ...
        moneyAvailable.setText("Cash: " + decimalPoint(cash));
        displayQuarter.setText("Quarter " + quarter);
        updatePortfolio();
        updateLayout();
    }
    ...
    private void updatePortfolio()    {
        if(!myStocks.isEmpty()) {
            displayPortfolio.setText("Your Portfolio: " + System.lineSeparator() +
                    myStocks.stream().collect(Collectors.groupingBy(Data::getName,
                            Collectors.counting())));
        }
        else    {
            displayPortfolio.setText("You don't own any stocks yet! So boring!");
        }
    }
    ...
    private void updateLayout() {
        ...
        displayQuarter.setCenter(size * 0.65, size * 0.1);
        displayPortfolio.setCenter(size * 0.75, size * 0.7);
    }
     ...
     * @return
     */
    private Double decimalPoint(Double number)  {
